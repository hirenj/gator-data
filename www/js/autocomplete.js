...
(function(window, document, undefined) {
	...
	AutocompleteProto.matchValue = function(val) {
		...
		if (!datalistSupported || !this.options.useNativeInterface) {
			if (val && !(results = this.cache["r-" + val])) {
				...
				results = this.values.map(function(val) {
					if (this.indexOf(val.toString()) >= 0) {
						return val;
					}
					return null;
				},matches).filter(function(val) { return val !== null;});
				this.cache["r-"+val] = results;
			}
	...
	AutocompleteProto.showValues = function(values) {
		...
		for (i=0, len=values.length; i<len; i++) {
			html[h++] = "<li data-idx='" + i + "' class='aLim'>" + values[i].toString().replace(rRgx, rStr) + "</li>";
		}
		html[h++] = "</ul>";
	...
	AutocompleteProto.addValues = function(values) {
		...
		if (values && values.length) {
			...
			var currentValuesString = currentValues.map(function(v) { return v.toString() });
			for (var i=0, value; (value=values[i]) !== undefined; i++) {
				if (value && indexOf(currentValuesString, value.toString()) === -1) {
